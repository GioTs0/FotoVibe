...
def create_app(settings=None, store=None, task_store=None):
    ...
    def pinned_photo_ids():
        """Photos an admin put on the stream by hand, as the latest pin event.

        Pins come and go during an evening, and the object store never mutates,
        so each click appends an event and the newest one for a photo wins. The
        state rides in the object metadata, the way reactions do, which keeps
        this to a single listing instead of a read per pin.
        """
        latest = {}
        for obj in store.list_prefix("pins/"):
            metadata = obj.metadata or {}
            photo_id = metadata.get("photo_id")
            if metadata.get("kind") != "pin" or not isinstance(photo_id, str):
                continue
            try:
                valid_id(photo_id)
            except HTTPException:
                log.warning("invalid_pin_marker object=%s", obj.name)
                continue
            recorded_at = metadata.get("recorded_at")
            recorded_at = recorded_at if isinstance(recorded_at, str) else obj.created
            active = metadata.get("active") not in {False, "0", "false", "False"}
            ...
            "photos": [
                {
                    ...
                    # The screens rank the evening's most celebrated photos
                    # themselves, so they need the comment count as well.
                    "comments": (entry.get("interactions") or {}).get("comments_count", 0),
